...
### Running app
1. Import to SailfishOS IDE (aka Qt Creator): File -> Open File or Project... -> helloworld-pro-sailfish/harbour-sailseries.pro
...
3. Configure build type (toolbar icon "harbour-sailseries Debug"): "i486-x86" & "Debug" & "Deploy as RPM package" & "src (on Mer device)"
...
### Running tests
* Option 1: In the emulator console, just run `/usr/share/tst-harbour-sailseries/runTestsOnDevice.sh`
...
* Option 2: Inside SailfishOS IDE
 ...
 2. Set "Alternate executable on device:" to `/usr/share/tst-harbour-sailseries/runTestsOnDevice.sh`
...
6. Final binaries will be located in RPMS folder and will look like `harbour-sailseries-0.1-1.armv7hl.rpm` Submit this RPM to harbour
...
5. Exit the virtual machine and then copy the package to the phone:
 ...
 scp -i ~/SailfishOS/vmshare/ssh/private_keys/SailfishOS_Device/nemo RPMS/harbour-sailseries-0.4-4.armv7hl.rpm nemo@ip.address.of.phone:RPMS/

6. Connect to the phone and install the package:

 ssh -i ~/SailfishOS/vmshare/ssh/private_keys/SailfishOS_Device/nemo nemo@ip.address.of.phone pkcon install-local RPMS/harbour-sailseries-0.4-4.armv7hl.rpm
...
1. Project structure
    ...
    * .yaml file (and .spec generated from .yaml) defines two subpackages:
        * harbour-sailseries - contains just the app files. It is something you will submit to app store
        * harbour-sailseries-tests - contains just the test binary and test files. For development only
...
2. Code structure
    * Note how tst_....qml files import the main project files. When running in device you want to import from final QML location that in this case is `../harbour-sailseries/qml/pages` relative from the test project deployment folder. When editing test file on desktop QtCreator sure has no idea where the main files are located, so during desktop editing I uncomment `import ../src/qml/pages` line to make QtCreator code completion work.
3. Further links
